package ru.geekbrains.homework.Lesson8Homework.Server;
    ...
    private void authTimer() {
        ...
        new Thread(() -> {
            while (true) {
                ...
                if (current - start == 15000 && name == null) {
                    try {
                        outputStream.writeUTF("Время авторизации истекло...");
                        socket.close();
                        break;
                    } catch (IOException e) {
                        throw new ChatServerException("Ошибка отключения.", e);
                    }
                }
            }
        })
